...
# Joined SKT dataset
SKT <- read.csv(file.path("SKT", "filesToUpload", "sktJoined.csv")) %>%
...
numberOfStations <- SKT %>% distinct(StationCode, LatitudeTheoretical, LongitudeTheoretical) %>% drop_na %>% nrow()
```
...
```{r, map image, fig.pos="H", out.width="100%", fig.cap=paste("The geographic range of the SKT Survey covers a total of", numberOfStations, "primary sampling stations, each represented by a point on the map. The stations are color-coded based on their relationship with the SKT index calculation: blue `core` stations that are included in the calculation, and red `non-core` station(s) that is/are not included.")}
knitr::include_graphics("sktMap.png")
```

## Field Sampling Methods

The SKT is a standard Kodiak trawl design.

**Net:** A weighted foot-rope and head-rope with floats allows the trawl to fish the top 1.8 meter of the water column.
\vspace{-3truemm}

  1. Net length: 19.8 m
  2. Mouth opening (fully opened): 7.62 m by 1.83 m (an area of 13.9446 $m^{2}$) 
  3. Mesh size: variable; from 2" knotted stretched mesh at the mouth, decreasing by $1/2^{"}$ across a series of 5 panels to $1/4^{"}$ knotless stretched mesh at the cod-end. The cod end mesh size has changed several times over the years, specifically in 2004 and 2007 (Table 4). New nets were incorporated in 2014 but were made to specifications of the previous nets.
...
## FILL in missing data for flow meter equipment above
...
```{r, schema, fig.pos="H", out.width="100%", fig.cap="The relational schema between the relational tables in the SKT database, used to produce the `sktJoined` table."}
knitr::include_graphics(file.path("skt", "sktRelationalSchema.png"))
```
...
SKT %>% 
    ...
    slice(1, n()) %>% 
    transmute(SampleDate, StationCode, 
              Latitude = LatitudeTheoretical, 
              Longitude = LongitudeTheoretical) %>%
