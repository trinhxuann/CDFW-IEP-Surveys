...
# This means that there are fractional counts in there. For now, Lauren, Vanessa, and Trinh
# have decided that there is no rounding required. This may need to be revisited
# in the future.
...
# Plot of median + 95% quantile and year of interest to check mean FL per survey
geometricMeans %>% 
  left_join(meanForkLength,
            by = c("year", "Survey")) %>% 
  group_by(year, threshold20) %>% 
  mutate(firstOccurence = ifelse(row_number() == 1 & threshold20 == T, T, NA)) %>% 
  group_by(Survey) %>% 
  mutate(medianForkLengthYear = median(meanForkLength, na.rm = T),
         forkLength2.5 = quantile(meanForkLength, na.rm = T, 0.025),
         forkLength97.5 = quantile(meanForkLength, na.rm = T, 0.975)) %>% 
  filter(Survey < 10) %>% 
  {
    ggplot(., aes(Survey, medianForkLengthYear)) +
      # geom_point(aes(color = "Yearly Average"), size = 3) +
      geom_ribbon(aes(ymin = forkLength2.5, ymax = forkLength97.5, fill = "Yearly Median"), alpha = 0.25) + 
      geom_line(aes(color = "Yearly Median")) +
      geom_point(data = filter(., year == 2018),
                 aes(Survey, meanForkLength, shape = factor(year)), size = 5, 
  ...
  labs(x = "Survey",
       ...
       fill = "",
       shape = "") +
  scale_fill_manual(values = c("#E5E2E2")) +
...
index %>% 
  select(-c(indexSurveyCount, previousMeanFL)) %>% 
  saveRDS(file = file.path("docs", "20 mm index", "20mmIndexCalculations.RDS"))
...
geometricMeans %>% 
  ...
  select(Year = year,
         ...
         MeanForkLength = meanForkLength,
         IndexSurveys = indexSurveys) %>% 
  ungroup() %>% 
  complete(Year, Survey) %>% 
  saveRDS(file = file.path("docs", "20 mm index", "20mmFLTable.RDS"))
