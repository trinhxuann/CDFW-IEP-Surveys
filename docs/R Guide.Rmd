...
author: "Trinh Nguyen, James R. White"
date: "April 25, 2022"
output:
  html_document:
    fig_caption: true
...
The topics covered in this guide will center around the Smelt Larval Survey publication process. Most functions used in that process will be featured here. we have assumed that if you are reading this, that you have just began dipping your toes into R. Our goals of this guide are to provides you with enough background and information to: 1) begin learning R join the dark side and 2) begin understanding the logic behind the manipulations in the SLS publication system.
...
An R script can contain code *and* comments. Comments are narratives that the authors write within the script. A line of comment is preceded by the symbol `#` followed by any text. It is best practice to comment your code as you go to help explain your approaches for the future you or your users. we have learned that it *is* worth making the extra effort to write in complete sentences and thoughts, because the you that is coding a section is likely smarter than the you that will be reading that section.
...
Converting dates is even more quirky: we will discuss the conversion of character values to dates here
...
# Again, we recommend reading through this much more [indepth guide](https://www.stat.berkeley.edu/~s133/dates.html)
```
...
For most users, we recommend simply familiarizing yourself with the `lubridate` package, discussed in the [Dates section of this document](#dates), which simplifies working with dates in R.
...
R can read in many different data formats, with the most common being Excel, text, and csv files. we will present the tidyverse variants below, but there are base R variants as well. Before any analysis, the user should fully understand if their datasets have been read in correctly.
...
The `col_only()` is a function that outputs the necessary formats required by the `col_type()` argument in various tidyverse read verbs, i.e.,, all the read sections below except for the Access section. Although this seems as an extra step, we believe it is a best practice to learn as you do not want to risk perpetuating an error that stems from the very beginning of your analysis.
...
# we want to pull flow gauge data from a USGS gauge in Middle River (11312676)
...
This section will somewhat mirror the previous section on reading in data into R, however, we will not talk about writing to Access here. R supports writing to many different file types, such as Excel, text (csv), and its two native formats, RDS and RDA.
...
There are functions in base R to write text files, but the `readr` versions are faster, will never write row names, and will always use "UTF-8" encoding (decreases the chances of encoding errors). Since you will likely be using this package to read in data, we recommend that you use the writing functions from this package also.
...
Again, these are our preferred formats. The advantages are the same as previously described in the reading section: 1) your dataset can be compressed to save on disk space, 2) fast read speed into R, 3) any R object can be saved and not just data tables, and very importantly 4) the data structure for your object(s) is preserved.
...
Before we get into the data manipulation operations, we would like to touch on this debate on base R vs 'tidyverse'. These are two main schools of coding in R. Base R refers to the coding infrastructure that is available immediately after installing the program; while tidyverse is "an opinionated collection of R packages" that introduces alternate and independent infrastructures to coding in R (tidyverse.org). It is our opinion that climbing the learning wall of R is much easier with tidyverse than base R. The main advantage is that the grammar and syntax of the tidyverse approach reads like an English sentence and is much more intuitive. As such, the operations below will be primarily in tidyverse but feel free to explore other approaches, such as base R or `data.table`. At the end of the day, everyone will have their own preferred style for coding--the list we provide below are our preference and may not be the absolutely best approach. You can load the tidyverse packages via:
...
The pipe assignment is a tool used to link several operations together in R. There are two main variants, one from base R and one from tidyverse. Although both tries to accomplish the same linking mechanism, their approach is slightly different. we cannot go into details on these differences as we have yet to extensively use the base R variant. Instead, we will first discuss the general concept of both and then describe the specifics of the tidyverse variant.
...
# Start off with a df of dateTime in character format, parse it correctly, then calculate various metrics. The data.frame() and mutate() functions will be discussed in more details in subsequent sections, but we are using them here for brevity
...
Overall, we would recommend *always* using the `%in%` function over the `==` unless you want an *exact* match, e.g., matching a single value. Although the negative form is slightly less intuitive, the flexibility of `%in%` is worth it.
...
# From the original data frame, we only want column A and C
data.frame(columnNameA = c(5, 6, 7),
           columnNameB = c("A", "B", "C"),
           columnNameC = c("D", "E", "F")) %>%
...
When creating your columns, you may need to employ conditional programming. There are two main methods for doing this, one in base R and one in tidyverse. we will go over the base R one first given that it is the more commonly used variant: the `ifelse()` function.
...
There are other joins, such as `right_join()`, `anti_join()`, and `semi_join()`, but we will not cover them here as they are special cases that are rarely used. You can consult their help page, as their intuition is same as the joins discussed here.

### Data structure

A data structure optimized for data storage may not be the optimal structure for data analysis. Specifically, the CDFW surveys are stored as relational databases, which is a collection of data tables that are connected to one another across shared columns following predefined relationships. Before data analysis, you must first resolve these relationships using various data joins, filters, selects, and overall manipulations (operations outlined in this guide). This crafting process can be tricky. The structure of your final data frame will dictate both the types of analysis you can do and how easy it will be to carry out those analyses. Although there are various approaches, we recommend the tidyverse's approach of creating your data frame in a "tidy" format.

#### Tidy data

There are 3 rules which define the tidy format:

1. Each variable must have its own column.
2. Each observation must have its own row.
3. Each value must have its own cell.

![Following three rules makes a dataset tidy. Variables are in columns, observations are in rows, and values are in cells. Taken from Figure 12.1 in R for Data Science, https://r4ds.had.co.nz/tidy-data.html#fig:tidy-structure](tidy-1.png)

Two main benefits of this approach are 1) you are following a standardized way to structure your dataset that is interpretable across data users and 2) you can effectively utilize the functions within the tidyverse package. Data frames that do not satisfy these three rules would be considered "non-tidy" data. Here's an example:

```{r messy}
data.frame(Date = rep("9/4/2009", 4),
           Station = c(410, 504, 601, 802),
           Secchi = c(0.88, 0.78, 0.95, 0.91),
           AmericanShad = c(5, 18, 34, 1),
           LongfinSmelt = c(1, 0, 2, 5))
```

This is non-tidy data because the columns `American Shad` and `Longfin Smelt` are arranged as separate columns (i.e., variables), when, in fact, they actually contain values of the same variable, `species`. This is problematic because your variables are housed and must be called in three different ways: 1) as columns (Date, Station, Secchi), 2) as headers (AmericanShad, LongfinSmelt), and 3) as values (catch counts). This is an issue for regression analysis in R, for example, where the predictor and response variables must all be in their own columns.

This data frame can be made into the tidy format via `pivot_longer()`, discussed in ["Pivoting dfs"](#pivoting):
...
data.frame(Date = rep("9/4/2009", 4),
           ...
           AmericanShad = c(5, 18, 34, 1),
           LongfinSmelt = c(1, 0, 2, 5)) %>%
  pivot_longer(c(AmericanShad, LongfinSmelt),
               names_to = "Species",
               values_to = "Catch")
```

In this form, all variables are now their own columns and you have a consistent and single way to refer to each of your variables (as columns). This is now a tidy dataset as variables are in columns, observations are in rows, and values are in cells. Continuing with our example, a regression analysis in which the response variable is catch across multiple species is now possible. This tidy form also allows you to easily leverage other tidyverse verbs to further manipulate your dataset for additional analyses, e.g., `filter()`, `arrange()`, `rename()`, `select()`, etc.

For CDFW survey data, we highly recommend you provide all flatfiles and data be provided in the tidy format (or at least provide another copy in tidy format) to facilitate a more ready-to-use product for the end users. Again, the relational database structure is optimized for data storage and not for data analysis; a tidy format reduces the data wrangling required from the end users, saving users front-end work *and* reduces the potential for introduced errors caused by users who are less familiar with the data.

### Pivoting dfs {#pivoting}
...
The arguments of `pivot_longer()` begin with what columns will be aggregated by the pivot. Here, we are negating the year column, which essentially says to pivot all columns other than `year`. You could theoretically do `c(DS, CHN, LFS, GS)` as well, but that is less effecient and is restrictive if additional catch columns are added after your code.
...
You can do this using the `summarize()` function from tidyverse. As the name suggests, this function will return to you a *summary* data table of the operations you are asking for--this summary table is important as it means the dimensions of your data frame is changed. The argument format of the function is similar to `mutate()` and supports a wide range of functions. For this specific example, we are asking for the total sum of `columnNameA`, mean of `columnNameC`, and the median of `columnNameD`. The returned table is a simple table with one row as you are asking for the *summary* statistics across the entire dataset. You should also note that the function only keeps the columns that are you making calculations for, i.e., `columnNameB` is not in the final data frame (if you wanted this column to be preserved, use mutate instead).
...
This layered drawing process is a critical component in understanding how ggplot works in R. This concept is the only thing we will discuss here. Obviously, there are many other features to `ggplot` that allows you to have fine control on all aspects of the figure. There are many resources online, but our two favorites are: [first example](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html) and [second example](https://www.datanovia.com/en/blog/ggplot-examples-best-reference/).
