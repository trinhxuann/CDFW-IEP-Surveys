...
SLSIntegrate <- function(filePath = NULL) {
  ...
  filesToRead <- list.files(path = filePath, pattern = ".csv")
  SLSTables <- list()
  ...
  SLSTables$`Tow Info` <- readr::read_delim(file.path("data-raw", "SLS", "TowInfo.csv"), delim = ",",
                                            col_types =
                                              readr::cols_only(
                                                Date = readr::col_date("%Y-%m-%d"),
                                                Station = readr::col_character(),
                                                Tow = readr::col_integer(),
                                                # Time = readr::col_time("%Y-%m-%d %H:%M:%S"),
    ...
    dplyr::mutate(Date = as.Date(Date),
                  Time=lubridate::ymd_hms(Time, tz="America/Los_Angeles"),
  ...
  SLSTables$`Water Info` <- readr::read_delim(file.path("data-raw", "SLS", "WaterInfo.csv"), delim = ",",
                                              col_types =
                                                readr::cols_only(
  ...
  SLSTables$`20mm Stations`<-readr::read_delim(file.path("data-raw", "SLS", "Station_Lookup.csv"), delim = ",",
                                               col_types =
                                                 readr::cols_only(
                                                   ID = readr::col_double(),
                                                   Station = readr::col_character(),
